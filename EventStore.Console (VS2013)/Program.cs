...
namespace EventStore.Console
{
    class Program
    {
        ...
        {
            var projectionsManager = new ProjectionsManager(new NoopLogger(), 
                        httpEndPoint);
            String allProjections = projectionsManager.ListAll(
        ...
        {
            using (var eventStoreConnection = 
                EventStoreConnection.Create(tcpEndPoint))
            {
                ...
                foreach (var session in sessionInfoList)
                {
                    ...
                    eventDataForAllSessions.Add(
                        new EventData(Guid.NewGuid(), 
                        "Session", isJsonFormat, json, noMetaData));
                }
                eventStoreConnection.AppendToStream(conferenceName, 
                    ExpectedVersion.Any, eventDataForAllSessions);
            }
        ...
        private static void GetEvents(IPEndPoint tcpEndPoint, string roomName)
        {
                ...
                EventStoreConnection.Create(tcpEndPoint))
            {
                ...
                StreamEventsSlice stream = eventStoreConnection.ReadStreamEventsBackward(
                    "Room-" + roomName, 
                    -1, int.MaxValue, resolveLinkTosAsWeOnlyStoreOneInstanceOfEventData);
                Dictionary<String, SessionInfo> sessionList = 
                ...
                foreach (ResolvedEvent ev in stream.Events)
                {
                    ...
                    if (!sessionList.ContainsKey(si.StartTime))
                        sessionList.Add(si.StartTime, si);
                }
